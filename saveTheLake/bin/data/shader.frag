...
void main()
{
    ...
    vec2 pixel = 1.0 / iChannelResolution.xy;
    float max_po = 5.0;
    vec2 po = max_po * vec2(xo + xt, yo + yt) * pixel;
    
    //vec2 tc = gl_FragCoord.xy / iResolution.xy;
    vec2 tc = texCoordVarying;
    vec3 bg = texture2D(tex0, tc + po).rgb;
    
    vec3 color = texture2D(tex0, tc).rgb;
    float blurAmnt = 0.1;
    //vec3 color = 1.0 * texture2D(tex0, tc + vec2(blurAmnt * -4.0, 0.0)).rgb;
